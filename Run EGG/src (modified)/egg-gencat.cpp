...
int vif_main(int argc, char* argv[]) {
    ...
    bool no_agn = false;         // do not generate fluxes and lines from agn origin (Xavi)
    bool no_random = false;      // disable all randomization of parameters
                                 // (just passive, M*, z, and position + pos angle)
    bool no_addlines = false;    // do not add lines to spectra (by LuciaP)
    ...
    // Xavi: QSO library (from qsogen, Temple+21)
    //std::string qso_lib_file = egg_share_dir+"qso_lib_temple21.txt";
    std::string qso_lib_file = egg_share_dir+"qso_lib_temple21.fits";
    //double opt_lib_min_step = 0.001; // um original
    //double opt_lib_min_step = 0.00005; // um  --> 0.5AA
    double opt_lib_min_step = -1.; // Micol: if negative there will be no rebinning at all
    ...
    // Xavi: File with Type 2 AGN narrow lines (Feltre+16)
    std::string agn2_lines_file = egg_share_dir+"AGN2_lines_feltre16.txt";
    ...
    // Xavi: name(agn2_lines_file, "agn2_lines"), no_agn
    // ---------------------------
    read_args(argc, argv, arg_list(
        ra0, dec0, area, mmin, maglim, zmin, zmax, name(bin_dz, "dz"), min_dz, max_dz,
        name(bin_dm, "dm"), ms_disp,
        no_pos, no_clust, no_flux, no_stellar, no_dust, no_passive_lir, no_random, no_nebular, no_agn, no_addlines,
        ...
        verbose, name(tseed, "seed"), name(tcosmo, "cosmo"),name(agn2_lines_file, "agn2_lines"),
    ...
    if (opt_lib_file.empty()) {
        if (igm == "constant") {
            opt_lib_file = egg_share_dir+"opt_lib_fast_hd.fits";
        } else {
            opt_lib_file = egg_share_dir+"opt_lib_fast_hd_noigm.fits";
        }
    }
    ...
    if (trim_filters || filter_flambda || filter_photons || filter_normalize) {
    ...
    // Modified by Micol: -0.23 dex for Chabrier, -0.19dex for Kroupa
    //if (opt_lib_imf == "chabrier" || opt_lib_imf == "kroupa") {
    //    opt_lib.sed *= e10(-0.2);
    double salp2chab = -0.23;
    double salp2krou = -0.19;
    if (opt_lib_imf == "chabrier") {
        opt_lib.sed *= e10(salp2chab);
    } else if (opt_lib_imf == "kroupa") {
        opt_lib.sed *= e10(salp2krou);
    ...
    // Micol: conversion from air to vacuum wavelengths above 2000A = 0.2nm
    // Make a copy of wavelengths (the SEDs remain the same)
    vec1u idf2 = mult_ids(opt_lib.use, where_first(opt_lib.use));
    vec1d olam2 = opt_lib.lam(idf2[0],idf2[1],_);

    // Find index of lambda>2000A
    uint_t id3 = where_first(olam2 >= 0.2);
    //print(" id3 = ",id3," lambda = ", olam2(id3));
    if (id3 != npos) {
        // Define new lambda
        vec1d vaclam;
        // Copy first wavelengths<2000A that are already in vacuum
        if (id3 != 0) {
	    append(vaclam, olam2[_-(id3-1)]);
        }
        // veclam2 contains all the lambda corrected from air to vacuum 
	// Constants in [(10-6 m)**-2] from Ciddor 1996,
	// Applied Optics, Vol. 35, Issue 9, pp. 1566-1573.
        double k0 = 238.0185;
        double k1 = 5792105.0;
        double k2 = 57.362;
        double k3 = 167917.0;
        vec1d sigma2 = sqr((1./olam2)); // sqr((1.E4/olam2) if lambda in A
        vec1d nas = (k1/(k0-sigma2)+ k3/(k2-sigma2))/1.0e8+1.0;
        vec1d vaclam2 = olam2*nas;
        // append only the ones above 2000A 
        append(vaclam, vaclam2[(id3)-_]);

        for (uint_t iuv : range(opt_lib.use.dims[0]))
        for (uint_t ivj : range(opt_lib.use.dims[1])) {
            if (!opt_lib.use(iuv,ivj)) continue;
            opt_lib.lam(iuv,ivj,_) = vaclam;
        }
    }
    ...
    // Type 1 quasar library, from Temple+21 (Xavi)
    // qsogen: https://github.com/MJTemple/qsogen/tree/cdcbecbd34f8accf29f57baeb771124da2b4cb31#introduction
    
    /*
    //Columns: 1)log10 Rest frame wavelenght [AA] 2)log10 Flux density per unit wavelength [erg/s/cm^2/AA](
     struct {
     vec1f lam, sed;
     } qso_lib;
     ascii::read_table(qso_lib_file, qso_lib.lam, qso_lib.sed);
     */
    
    struct {
        vec2f lam, sed;
        vec1f real_Mi, beslope_Mi, et, L2500;
    }qso_lib;
    
    fits::read_table(qso_lib_file, ftable(
        qso_lib.lam, qso_lib.sed, qso_lib.real_Mi, qso_lib.beslope_Mi, qso_lib.et, qso_lib.L2500
    ));
    
    //const double L2500_qsotemplate = mean(qso_lib.L2500); //average log10 rest-frame luminosity at 2500 AA of the qso templates, [erg/s/Hz]
    //const double Mi_qsotemplate    = mean(qso_lib.real_Mi); //average i absolute magnitude of the qso templates
    const double L2500_qsotemplate = 31.085; //average log10 rest-frame luminosity at 2500 AA of the qso templates, [erg/s/Hz]
    const double Mi_qsotemplate    = -25.77; //average i absolute magnitude of the qso templates
    
    // Monochromatic UV luminosity at 2500 AA from [2-10]keV band X-ray luminosity,
    auto get_qso_sed = [&](const double Lx210, vec1f& sed, vec1f& emline_type, const int i ) { //Lx in log [erg/s], sed in log [erg/s/cm^2/Hz]
        const double Gamma = 1.8;
        const double E = 2./4.1357e-18; //kev to Hz
        const double E1 = 2./4.1357e-18; //kev to Hz
        const double E2 = 10./4.1357e-18; //kev to Hz
        const double Lx2 = log10((2-Gamma)*(pow(E,1.-Gamma))/(pow(E2,2.-Gamma) - pow(E1,2.-Gamma))) + Lx210; //L_2kev in log [erg/s/Hz], Eq 6 in Bisogni et al 2021
        const double slope = 0.58; //slope and intercept of the Lx-Luv relation
        const double intercept = 8.56;
        const double disp = 0.24*randomn(seed);
        // const double disp = 0.0;
        const double L2500 = (Lx2-intercept+disp)/slope; //log10 rest-frame luminosity at 2500 AA of the AGN according to the Lx-Luv relation [erg/s/Hz]
        const double newMi = Mi_qsotemplate -2.5*(L2500 - L2500_qsotemplate);
        emline_type[i] = 0.183*(newMi + 27);
        emline_type[i] = round(emline_type[i]*10)/10; //round to 1st decimal
        uint_t qso_index;
//        if (emline_type < -2){
//            qso_index = 0;
//        }
//        else if (emline_type > 3){
//            qso_index = 50; //in general, qso_index = qso_lib.et.size() - 1;
//        }
//        else {
//            qso_index = 10*(emline_type+2); //in general, qso_index = (qso_lib.et.size() - 1)/(qso_lib.et(qso_lib.et.size() - 1) - qso_lib.et(0))*(emline_type-qso_lib.et(0));
//        }
        emline_type[i] = clamp(emline_type[i],-2.,3.);
        qso_index = 10*(emline_type[i]+2);
        sed = qso_lib.sed(qso_index,_);
        sed += L2500 - L2500_qsotemplate;
    };
    ...
    struct {
        ...
        vec1f av_qso; //Added by Xavi, only for type-1

        vec1f tdust, ir8, fpah, mdust;

        vec1b passive;
        vec1u agn_type; //added by Xavi. Can be 0 (no AGN),1 or 2 (for type 1/2)
        ...
        vec1u opt_sed_bulge;
        vec1u opt_sed_disk;
        //vec1u opt_sed_agn; // Xavi: The AGN component will have a SED that is QSO from Temple21 for type 1, 0 + narrow lines for type 2, and 0 everywhere for galaxies
        vec2f igm_abs;
        ...
        vec1f avlines_disk;
        vec1f avlines_bulge;
        vec2f line_lum_nodust; //added by Micol
        vec2f line_lum_disk_nodust; //added by Micol
        vec2f line_lum_bulge_nodust; //added by Micol
        vec2f line_lum_noigm; //added by Micol
        vec2f line_lum_disk_noigm; //added by Micol
        vec2f line_lum_bulge_noigm; //added by Micol
        vec2f line_lum;
        vec2f line_lum_disk;
        ...
        vec1f agn2_vdisp;           //added by Xavi
        vec2f agn2_line_lum;        //added by Xavi
        vec2f agn2_line_lum_nodust; //added by Xavi
        vec2f agn2_line_lum_noigm; //added by Xavi
        vec1s agn2_lines;           //added by Xavi (name of the lines e.g. "n5_1240")
        vec1f agn2_line_lambda;     //added by Xavi (wavelenght of the lines in microns e.g. 0.12428)
        vec1f logU;                //added by Xavi (ionization parameter)
        vec1f Lx;                  //added by Xavi (intrinsic X-ray luminosity in the 2-10 keV band)
        vec1f Lacc;                //added by Xavi (accretion luminosity, used to escale the line luminosities)
        vec1f nh_nlr;               //added by Xavi (hidrogen density in AGN)
		vec1f uv_slope_nlr;        //added by Xavi (slope of the UV continuum in type 2 AGN)
        vec1f Mbh;                 //added by Xavi (Black Hole Mass)
        vec1f emline_type;         //added by Xavi (emission line type for type 1 QSO, parameter in Temple+21 model, range -2 to +3)
        vec1f NH;                  //added by Xavi (absorption column density in the X-ray spectrum, following Ueda+14, range 20 to 24)

        vec1f metallicity;  //added by Micol
        vec1f metallicity2; //added by Micol
        vec1f metallicity_agn2; //added by Xavi
        vec1f m_h2;         //added by Micol
        vec1f m_gas;        //added by Micol
    ...
    if (!input_cat_file.empty()) {
        ...
        if (ends_with(input_cat_file, ".fits")) {
            ...
            // Xavi: added read agn flag, Lx and Lacc, Mbh
            tbl.read_columns(fits::narrow, ftable(
                out.ra, out.dec, out.z, out.m, out.agn_type, out.Lx, out.Lacc, out.Mbh, out.NH
            ));
            ...
            // Xavi: Read agn flag and Lacc if input file is ascii
            ascii::read_table(input_cat_file, out.id, out.ra, out.dec, out.z, out.m, out.passive,out.agn_type, out.Lacc);
        }
    ...
    const vec1u ida1 = where(!out.passive && out.agn_type==1); // added by Xavi
    const vec1u ida2 = where(!out.passive && out.agn_type==2); // added by Xavi
    const vec1u idp = where(out.passive);
    const vec1u idp1 = where(out.passive && out.agn_type==1); // added by Xavi
    const vec1u idp2 = where(out.passive && out.agn_type==2); // added by Xavi
    const vec1u idagn = where(out.agn_type==1 || out.agn_type==2); // added by Xavi
    const vec1u idagn1 = where(out.agn_type==1); // added by Xavi
    const vec1u idagn2 = where(out.agn_type==2); // added by Xavi
    const vec1u idgal = where(out.agn_type==0); // added by Xavi
    const uint_t nactive = ida.size();
    const uint_t nactive1 = ida1.size(); // added by Xavi
    const uint_t nactive2 = ida2.size(); // added by Xavi
    const uint_t npassive = idp.size();
    const uint_t npassive2 = idp2.size(); // added by Xavi
    const uint_t ngal = out.z.size();
    const uint_t nagn = idagn.size(); // added by Xavi
    const uint_t nagn1 = idagn1.size(); // added by Xavi
    const uint_t nagn2 = idagn2.size(); // added by Xavi
...
if (!no_stellar) {
    ...
    auto calzetti2000 = vectorize_lambda([&](double l) {
        ...
        //Returns k(lam)/Rv (=A(lam)/Av), assuming Rv = 3.1
    ...
    // All nebular lines are calculated and saved in solar luminosity
    if (!no_nebular) {
        if (verbose) {
            note("generating nebular emission lines");
        }
	// Micol: NB these lines wavelengths are in air change this
	...
	//        append(out.line_lambda, vec1f{0.65628,  0.48613, 0.43405,  0.41017,  0.65835,   0.65480,   0.50068,   0.49589,   0.37274,   0.12157});
        ...
        // added by Micol: UV to NIR lines wavelengths are now in **vacuum**
        append(out.lines,       vec1s{"halpha", "hbeta", "hgamma", "hdelta", "hepsilon", "h8", "h9", "h10"});
        append(out.line_lambda, vec1f{0.656461, 0.486268, 0.434168, 0.410289, 0.39712, 0.38902, 0.38365, 0.379898 });
        append(out.lines,       vec1s{"palpha", "pbeta", "pgamma", "pdelta", "pepsilon", "p9", "p10"});
        append(out.line_lambda, vec1f{1.87561, 1.28216, 1.09411, 1.00521, 0.95486, 0.92315, 0.901739}); 
        append(out.lines,       vec1s{"n2_6583", "n2_6548", "o3_5007", "o3_4959", "o2_3727", "lyalpha"});
        append(out.line_lambda, vec1f{0.658527,   0.654986,   0.500824,   0.496030,   0.372849,   0.12157});
        append(out.lines,       vec1s{"s2_6731", "s2_6717", "s3_9531", "s3_9069", "ne3_3869"});
        append(out.line_lambda, vec1f{0.673271, 0.671832, 0.953461, 0.907149, 0.386986});

        // added by Micol: UV to NIR lines wavelengths in **air** (uncomment this and comment vacuum wavelengths if necessary)
        // notice that Lyalpha is only in vacuum
        //append(out.lines,       vec1s{"halpha", "hbeta", "hgamma", "hdelta", "hepsilon", "h8", "h9", "h10"});
        //append(out.line_lambda, vec1f{0.65628, 0.48613, 0.43405, 0.41017, 0.39701, 0.38891, 0.38354, 0.379791 });
        //append(out.lines,       vec1s{"palpha", "pbeta", "pgamma", "pdelta", "pepsilon", "p9", "p10"});
        //append(out.line_lambda, vec1f{1.87510, 1.28181, 1.09381, 1.00494, 0.95460, 0.92290, 0.90153}); 
        //append(out.lines,       vec1s{"n2_6583", "n2_6548", "o3_5007", "o3_4959", "o2_3727", "lyalpha"});
        //append(out.line_lambda, vec1f{0.658345,   0.654805,   0.5006843,   0.4958911,   0.372732,   0.12157});
        //append(out.lines,       vec1s{"s2_6731", "s2_6717", "s3_9531", "s3_9069", "ne3_3869"});
        //append(out.line_lambda, vec1f{0.6730815, 0.6716440, 0.95306, 0.90686, 0.386876});
	
        out.line_lum_bulge.resize(ngal, out.lines.size());
        out.line_lum_disk.resize(ngal, out.lines.size());

	// added by Micol:  
        out.line_lum_bulge_nodust.resize(ngal, out.lines.size());
        out.line_lum_disk_nodust.resize(ngal, out.lines.size());
        out.line_lum_bulge_noigm.resize(ngal, out.lines.size());
        out.line_lum_disk_noigm.resize(ngal, out.lines.size());
    
        const uint_t nlines = out.lines.size();
        ...
        //vec1d metal2 = replicate(9.07, ngal);
        //vec1d mu32 = (out.m-0.2) - 0.32*(log10(out.sfr) - 0.2); 
        vec1d mu32 = (out.m + salp2chab) - 0.32*(log10(out.sfr) + salp2chab); 
        // notice that 0.2 dex (now salp2chab - Micol) are subtracted to both stellar mass and sfr 
        // to convert to Chabrier IMF
	...
	// Micol: Comment/uncomment to select metallicity from EGG or from Curti
        // Metallicity from EGG:
        metal = clamp(metal, -0.6, 1.0);
        //metal2 = clamp(metal2, -0.6, 1.0);

        //Metallicity from Curti et al. 2020 (added by Micol):
        //Notice that stellar mass is with Chabrier IMF, so it has to be converted
        //I assume the conversion to be -0.2dex (now salp2chab - Micol), as assumed previously in this code
        //vec1d metal = replicate(1., ngal);
        //vec1d logm0 = 10.106+0.559*(log10(out.sfr) + salp2chab);
	//metal = 8.779 - (0.311/2.06) * log10(1+pow(pow(10,logm0 - (out.m + salp2chab)), 2.06));
	//metal -= 8.69;
        vec1d metal2 = replicate(1., ngal);
        vec1d logm0 = 10.106+0.559*(log10(out.sfr)-0.2);
	metal2 = 8.779 - (0.311/2.06) * log10(1+pow(pow(10,logm0 - (out.m - 0.2)), 2.06));
	metal2 -= 8.69;
        ...
        out.metallicity=metal;
        out.metallicity2=metal2;
        out.m_gas=mgas;
        out.m_h2=mh2;
        ...
        // Micol & Lucia: tweak to improve some line statistics
        out.avlines_disk *= interpolate(vec1d{1.3, 2.0, 1.8, 1.5}, vec1d{0,1,2,100}, out.z);
        if (!no_random) out.avlines_disk += 0.1*randomn(seed, ngal);
        // lines are not produced in bulge, so AVbulge is useless for now 
        ...
        // Velocity dispersion (Bezanson+18)
        out.vdisp = e10(0.33*out.m - 1.34);
        if (!no_random) out.vdisp *= e10(0.15*randomn(seed, ngal));
        ...
        out.line_lum_disk(_,l) = e10(1.017*(log10(out.sfr) + salp2chab) + 7.13); // deLooze+11
        ...
        out.line_lum_disk(_,l) = e10(1.053*(log10(out.sfr) + salp2chab) + 5.31); // Zhao+13
        ...
        // UV-optical-NIR lines
        // Original lines: Ha,Hb,Hg,Hd,NII6583,NII6548,OIII5007,OIII4959,OII3727,Lya
        // Added:
	// He,H8,H9,H10
        // Pa,Pb,Pg,Pd,Pe,P9,P10
	// SII6731,SII6717,SIII9532,SIII9069
	// NeIII3869
        ...
        // Case B recombination: Hb, Hg, Hd ratios with Ha
	//vec1d recomb = {0.35, 0.163, 0.0862};
        ...
        // Modified by Micol: Balmer lines, line ratios with Ha from Osterbrok & Ferland, pag 73, 
	// Case B, T=10000K 
        // Hb, Hg, Hd, He, H8, H9, H10, Pa, Pb, Pg, Pd, Pe, P9, P10 ratios with Ha
	vec1d recomb = {1./2.87, 1./6.16, 1./11.21, 1./18.16, 1./27.33, 1./39.31, 1./54.25,
	    1./8.15, 1./17.39, 1./31.68, 1./51.71, 1./77.99, 1./112.99, 1./155.13};
	for (uint_t dl = 0; dl < 14; ++dl) {
            out.line_lum_bulge(_,l+1+dl) = 0.0;
	    out.line_lum_disk(_,l+1+dl) = out.line_lum_disk(_,l)*recomb[dl];
	}
        ...
        auto poly4 = [](const vec1d& x, const vec1d& coef) {
            return coef[0] + x*coef[1] + sqr(x)*coef[2] + pow(x,3)*coef[3] + pow(x,4)*coef[4];
        };
	auto poly5 = [](const vec1d& x, const vec1d& coef) {
            return coef[0] + x*coef[1] + sqr(x)*coef[2] + pow(x,3)*coef[3] + pow(x,4)*coef[4] + pow(x,5)*coef[5];
        };
	...
	// [SII] 6717,6731: from Mannucci
	vec1d s2ha = poly4(metal, {-0.442, -0.360, -6.271, -8.339, -3.559}); 
	vec1d ls2 = lha*e10(s2ha);

	l = where_first(out.lines == "s2_6731");
        out.line_lum_bulge(_,l) = 0.0;
        out.line_lum_disk(_,l) = 0.42*ls2;

	l = where_first(out.lines == "s2_6717");
        out.line_lum_bulge(_,l) = 0.0;
        out.line_lum_disk(_,l) = 0.58*ls2;
	
	// [SIII] 9069,9531: from Mannucci
	// [SIII] from [SII]/Ha and the relation between S32=log([SIII]/[SII]) and R3=log([OIII]/Hb), see Mannucci+20
        // S32 from R3 in fig 4, no DIG correction, parabolic fit to new figure
	// The following line define o3hb with the recipe implemented by Mannucci (LP??)
	//vec1d o3hb = poly3(metal, {-0.277, -3.549, -3.593, -0.981});
	vec1d s32 = -0.28+0.4*o3hb+0.15*sqr(o3hb); 
	vec1d ls3 = ls2*e10(s32);

	// split the doublet assuming S9532/S9069=2.5
	l = where_first(out.lines == "s3_9531");
        out.line_lum_bulge(_,l) = 0.0;
        out.line_lum_disk(_,l) = 0.71*ls3;

	l = where_first(out.lines == "s3_9069");
        out.line_lum_bulge(_,l) = 0.0;
        out.line_lum_disk(_,l) = 0.29*ls3;

	// [NeIII] 3869: from Mannucci
	// [NeIII]/[OII]  linear approx of calibration, scaled to Curti+17:
	// met=7.2 -> [NeIII]/[OII]=0
	// met=8.8 -> [NeIII]/[OII]=-1.4
	// Calibration of [OII]/Hβ from Curti+19

	vec1d lo2 = out.line_lum_disk(_,where_first(out.lines == "o2_3727"));
	vec1d o2hb = log10(lo2/lhb);
	// The following line define o2hb with the recipe implemented by Mannucci
	//vec1d o2hb = poly5(metal, {0.435, -1.362, -5.655, -4.851, -0.478, 0.736}); 
        double hbha = log10(recomb[0]);

	l = where_first(out.lines == "ne3_3869");
        out.line_lum_bulge(_,l) = 0.0;
        out.line_lum_disk(_,l) = lha*pow(10,-1.5-0.5*(metal)+o2hb+hbha);
	
        // Apply reddening and IGM
        // Micol: Notice that the 3.33 is actually k(Halpha) = A(Halpha)/EBV,
        // where AV/RV = AV/4.05 = EBV
        // See e.g. Eq. 6 in https://ui.adsabs.harvard.edu/abs/2013ApJ...763..145D/abstract
        // The EGG output provides AV_LINES_DISK which is in fact A(Halpha)
        // To be checked if the code applies the correct values then...
        auto calzetti2000 = vectorize_lambda([&](double l) {
            // http://adsabs.harvard.edu/abs/2000ApJ...533..682C
            const double iRv = 1.0/3.33;
            if (l <= 0.63) {
        ...
        // added by Micol: save lines before applying dust extinction
        out.line_lum_disk_nodust = out.line_lum_disk;
        out.line_lum_bulge_nodust = out.line_lum_bulge;
        out.line_lum_disk_noigm = out.line_lum_disk;
        out.line_lum_bulge_noigm = out.line_lum_bulge;
        // added by Micol: here the IGM attenuation is applied to the luminosity,
        // but it is better to apply it together with the continuum to allow for
        // the asymmetry of Lya
        ...
        for (uint_t i : range(ngal)) {
            ...
            for (uint_t l : range(out.lines)) {
                if (out.line_lambda[l] < 0.0600) {
                    ...
                    // Micol & Lucia: for Ly-alpha the IGM should be applied only
                    // to the blue part, i.e. to half of the luminosity (equivalent
                    // to assuming the transmission is twice)
                    if (out.lines[l] == "lyalpha") {
                        igm_lines[l] = 0.5*(out.igm_abs(i,2)+1.);
                    } else {
            ...
            out.line_lum_disk_noigm(i,_)  *= e10(-0.4*out.avlines_disk[i]*dust_law);
            out.line_lum_bulge_noigm(i,_) *= e10(-0.4*out.avlines_bulge[i]*dust_law);
        }

        // added by Micol: unattenuated emission lines (no dust, no IGM)
        out.line_lum_nodust = out.line_lum_bulge_nodust + out.line_lum_disk_nodust;
        
        // added by Micol: attenuated emission lines (yes dust, no IGM)
        out.line_lum_noigm = out.line_lum_bulge_noigm + out.line_lum_disk_noigm;

        // Compute total luminosities
        out.line_lum = out.line_lum_bulge + out.line_lum_disk;
    }
    
    // Xavi: Narrow emission lines for type-2 AGN
    // -------------------------------------
	    
    if (!no_agn) {
        if (verbose) {
            note("generating AGN emission lines");
        }
        
        // NB these lines wavelengths are in vacuum (where is Ly alpha?)
        out.agn2_lines       = {"n5_1240", "c4_1550", "he2_1640", "c3_1908", "mg2_2796", "mg2_2803", "ne5_3426", "o2_3727", "ne3_3869", "he2_4680"};
        out.agn2_line_lambda = {0.12428,   0.1549485,  0.164042,   0.1907705, 0.279553,   0.280271,   0.342685,   0.372849,  0.387016,   0.468601};
        append(out.agn2_lines,       vec1s{"o3_4959", "o3_5007", "o1_6300", "o1_6363", "n2_6548", "n2_6583", "s2_6717", "s2_6731", "s3_6312", "s3_9069", "s3_9531"});
        append(out.agn2_line_lambda, vec1f{ 0.496030,  0.500824, 0.630205,   0.636554,  0.654986,  0.658527,  0.671832,  0.673271,  0.631206,  0.90711,   0.95332});
        append(out.agn2_lines,       vec1s{"h9", "h8", "hepsilon", "hdelta", "hgamma", "hbeta", "halpha"});
        append(out.agn2_line_lambda, vec1f{0.38365, 0.38902, 0.39712, 0.410289,0.434168, 0.486268, 0.656461});
        append(out.agn2_lines,       vec1s{ "p9"  , "pepsilon","pdelta", "pgamma", "pbeta", "palpha"});
        append(out.agn2_line_lambda, vec1f{0.92315, 0.95486,    1.00521,  1.09411,  1.28216,  1.87561});
        
        out.agn2_line_lum.resize(ngal, out.agn2_lines.size()); //arrays are initialized to 0 in vif
        out.agn2_line_lum_nodust.resize(ngal, out.agn2_lines.size());
        
        const uint_t agn2_nlines = out.agn2_lines.size();
        print("N lines (AGN Type 2): ",agn2_nlines);
        
        // Read table in 'agn2_lines_file' TO DO: move all these variables into an astructure called table_lines
        vec1f table_logU;
        vec1f table_metallicity;
        vec1f dust_to_metal; // Dust-to-metal ratio (will be fixed at xi = 0.3)
        vec1f hydrogen_density; // Hydrogen number density from table
        vec1f uv_spectral_slope; // UV spectral slope (will be fixed at al = -1.4)
        vec2f table_agn2_line_lum;
        ascii::read_table(agn2_lines_file, _, table_logU, _, table_metallicity, dust_to_metal, hydrogen_density, uv_spectral_slope, ascii::columns(out.agn2_lines.size(),table_agn2_line_lum));
		
        //table_metallicity -= 8.69; // in solar metalicity (as used in EGG for the nebular lines)
        table_metallicity -= 8.71;  // the solar metallicity from Gutkin16
        
        // Resize and intialize logU, agn2_vdisp, UV spectral slope
        out.logU.resize(ngal);
        out.agn2_vdisp.resize(ngal);
		out.uv_slope_nlr.resize(ngal);
        out.nh_nlr.resize(ngal);
        out.logU -= 99.9;
        out.agn2_vdisp -= 99.9;
        out.uv_slope_nlr -= 99.9;
		out.nh_nlr -= 99.9;
		
        // Define hydrogen number density, randomly choosen between lnh = 3 or lnh = 4)
        out.nh_nlr[idagn2] = randomi(seed, 3, 4, idagn2.size());
        
        // Define metallicity_agn2, which is a random number between 0(=solar) and max(metallicity)
        out.metallicity_agn2 = out.metallicity;
        //double max_met = max(out.metallicity);
        double max_met = max(table_metallicity);
        print("AGN metallicity between 0 and ", max_met);
        //const vec1u id_met = where(out.metallicity < 0);
        //out.metallicity_agn2[id_met] = max_met*randomu(seed, id_met.size());
        out.metallicity_agn2[idagn2] = max_met*randomu(seed, idagn2.size());
        
        for (uint_t ii : idagn2) { //Iterate over all type-2-AGN indexes
			//Define UV spectral slope, randomly choosen between -1.4 or -1.7 (one-line if-else)
			out.uv_slope_nlr[ii] = (randomu(seed) > 0.5) ? -1.4 : -1.7;
			 
			//Assign ionisation parameter (based on Fig 6 of Hirschmann et. al 2017)
            if (out.z[ii] >= 0 && out.z[ii] < 1){
                out.logU[ii] = -4.5 + (-2.5+4.5)*randomu(seed); // random number between -4.5 and -2.5 (inclusive)
            }
            if (out.z[ii] >= 1 && out.z[ii] < 2){
                out.logU[ii] = -4.0 + (-2.0+4.0)*randomu(seed); // random number between -4.0 and -2.0 (inclusive)
            }
            if (out.z[ii] >= 2){
                out.logU[ii] = -3.5 + (-1.5+3.5)*randomu(seed); // random number between -3.5 and -1.5 (inclusive)
			}
			out.logU[ii] -= 0.35; //Because of the different definitions of U in Feltre+16 and Hischmann+17 
            //out.logU[ii] = -5+0.5*randomi(seed, 0, 7); // TEST: Ionization parameter: without constrains (random number between -5 and 1.5 (inclusive) in multiples of 0.5)
			
            //Assign velocity dispersion for narrow lines
            //out.agn2_vdisp[ii] = 500 + (800 - 500)*randomu(seed); // OLD: Velocity dispersion: random number between 500 and 800
            //out.agn2_vdisp[ii] = 255 + 85*randomn(seed); // OLD: Velocity dispersion: Gaussian distribution with median 255 km/s and sigma 85 km/s (Zakamska 2003)
			out.agn2_vdisp[ii] = pow(10, 2.55+0.18*randomn(seed)); // UPDATE: Velocity dispersion: Lognormal distribution with median 355 km/s (Fig 6 of Menzel+16)
            if (out.agn2_vdisp[ii] <= 50){
                out.agn2_vdisp[ii] = 50;
            }
            if (out.agn2_vdisp[ii] >= 1200){
                out.agn2_vdisp[ii] = 1200;
            }

	        //Interpolate in metallicity and logU fixing all the other parameters
	        double logLsun = 33 + log10(3.839);
            uint_t low_U = lower_bound(table_logU, out.logU[ii]);
            uint_t upp_U = upper_bound(table_logU, out.logU[ii]);
            vec1u id_agn_low = where(table_logU[low_U] == table_logU && abs(dust_to_metal -0.3)< 1e-3 && abs(hydrogen_density-out.nh_nlr[ii])< 1e-3 && abs(uv_spectral_slope  - out.uv_slope_nlr[ii])< 1e-3);
            vec1u id_agn_upp = where(table_logU[upp_U] == table_logU && abs(dust_to_metal -0.3)< 1e-3 && abs(hydrogen_density-out.nh_nlr[ii])< 1e-3 && abs(uv_spectral_slope  - out.uv_slope_nlr[ii])< 1e-3);
            for (uint_t jj : range(out.agn2_lines)){ //Iterate over all emission lines
                double lum_Zinterp_low_U = interpolate(table_agn2_line_lum(id_agn_low,jj), table_metallicity(id_agn_low), out.metallicity_agn2[ii] );
                double lum_Zinterp_upp_U = interpolate(table_agn2_line_lum(id_agn_upp,jj), table_metallicity(id_agn_upp), out.metallicity_agn2[ii] );
                out.agn2_line_lum(ii,jj) = interpolate(lum_Zinterp_low_U, lum_Zinterp_upp_U, table_logU[low_U], table_logU[upp_U], out.logU[ii]);
                out.agn2_line_lum(ii,jj) += out.Lacc[ii]; //Add normalization (Accretion luminosity)
                out.agn2_line_lum(ii,jj) -= logLsun; //Convert to solar luminosities. Value extracted from include/vif/astro/astro.hpp
                out.agn2_line_lum(ii,jj) = e10(out.agn2_line_lum(ii,jj)); //Convert to linear
            }

        }
        
        //double lum_metal_interpol = interpolate(table_agn2_line_lum(id_agn,jj), table_metallicity(id_agn), out.metallicity_agn2[ii] );
        //double lum_logU_interpol = interpolate(table_agn2_line_lum(id_agn,jj), table_logU(id_agn), out.logU[ii] );
        //out.agn2_line_lum(ii,jj) = interpolate(lum_metal_interpol,lum_logU_interpol,out.metallicity_agn2[ii],
        

        vec1d agn2_dust_law = calzetti2000(out.agn2_line_lambda);
        //agn_dust_law[where_first(out.agn2_lines == "lyalpha")] = 0; // extinction already folded in
        // added by Micol: save lines before applying dust extinction
        out.agn2_line_lum_nodust = out.agn2_line_lum;
        out.agn2_line_lum_noigm  = out.agn2_line_lum;
        
        // All the agn lines have lambda > 0.1216, so it goes directly to the last else (igm_agn2_lines[l] = 1.0)
        for (uint_t i : range(ngal)) {
            vec1d igm_agn2_lines(out.agn2_lines.size());
            for (uint_t l : range(out.agn2_lines)) {
                if (out.agn2_line_lambda[l] < 0.0600) {
                    igm_agn2_lines[l] = 0.0;
                } else if (out.agn2_line_lambda[l] < 0.0912) {
                    igm_agn2_lines[l] = out.igm_abs(i,0);
                } else if (out.agn2_line_lambda[l] < 0.1026) {
                    igm_agn2_lines[l] = out.igm_abs(i,1);
                } else if (out.agn2_line_lambda[l] < 0.1216) {
                    // Micol & Lucia: for Ly-alpha the IGM should be applied only
                    // to the blue part, i.e. to half of the luminosity (equivalent
                    // to assuming the transmission is twice)
                    if (out.lines[l] == "lyalpha") {
                        igm_agn2_lines[l] = 0.5*(out.igm_abs(i,2)+1.);
                    } else {
                        igm_agn2_lines[l] = out.igm_abs(i,2);
                    }
                } else {
                    igm_agn2_lines[l] = 1.0;
                }
            }

            out.agn2_line_lum(i,_)       *= e10(-0.4*out.avlines_bulge[i]*agn2_dust_law)*igm_agn2_lines; //Maybe improve in the future with agn_avlines
            out.agn2_line_lum_noigm(i,_) *= e10(-0.4*out.avlines_bulge[i]*agn2_dust_law); //Maybe improve in the future with agn_avlines
        }

        
    };
    // -------------------------------------
    // End of: Generate emission lines for agn
...
if (!no_flux) {
    ...
    out.flux_agn.resize(ngal, bands.size());
    out.rfmag_agn.resize(ngal, rfbands.size());
    ...
    vec1u save_sed_agn_start,   save_sed_agn_nbyte; //Xavi
    ...
    if (save_sed) {
        ...
        save_sed_agn_start.resize(ngal);
        save_sed_agn_nbyte.resize(ngal);
    }
    ...
    auto add_emission_line = [&](const vec1f& lam, vec1f& sed, double l0, double lum, double vdisp) {
        ...
        auto b = bounds(lam, l0 - 10*lw, l0 + 10*lw); //array<uint_t,2> bounds(T x1, U x2, vec v)
        if (b[1] == 0 || b[0] == n-1) return;
        if (b[0] == npos) b[0] = 0; //vif documentation: npos=unsigned integer whose value is the largest possible integer that the uint_t type can hold. Used as an error value, or the value to return if no normal value would make sense.
        if (b[1] == npos) b[1] = n-1;
    ...
    };//end of auto add_emission_line

    
    auto Temple_attenuation = vectorize_lambda([&](double l) {
        //Assuming a power law fit to the attenuation law from 'pl_ext_comp_03.sph' (https://github.com/MJTemple/qsogen/tree/main)
        //Returns k(lam)/Rv (=A(lam)/Av), assuming Rv = 3.1
        const double iRv = 1.0/3.1;
        
        if (l <= 0.195) {
            l = iRv*pow(10, 0.459) * pow(l, -0.77);
        } else {
            l = iRv*pow(10, 0.193) * pow(l, -1.15);
        }
        if (l < 0) l = 0;
        return l;
    });

    //out.av_qso = out.av_bulge;
    //out.av_qso = replicate(0.093,ngal);
    out.av_qso = random_pdf(seed,{0.025,0.073,0.125,0.174,0.224,0.272,0.325,0.423,0.772,0.973,1}, {0.621, 0.169, 0.060, 0.037, 0.031,0.027, 0.024, 0.010, 0.008, 0.005,0},ngal); //From Lusso+13 E(B-V)qso distribution
    out.av_qso = out.av_qso*3.1; //assuming Rv = 3.1
    // Resize and intialize emline_type
    out.emline_type.resize(ngal);
    out.emline_type -= 99.9;
    auto get_flux = [&](const vec1f& m, const vec1u& optsed, const vec2f& llum,
        ...
        for (uint_t i : range(ngal)) {
            ...
            vec1f rlam, rsed;
            if (cmp == component::agn) {
                vec1f rlamagn(qso_lib.lam(0,_).size());
                rlamagn = e10(qso_lib.lam(0,_));
                rlam = rlamagn;
                vec1f rsedagn(qso_lib.sed(0,_).size());
                if (out.agn_type[i] == 1) { // For type 1 AGN, select a template, reescale it, and apply attenuation
                    get_qso_sed(out.Lx[i], rsedagn, out.emline_type, i);
                    rsed = e10(rsedagn);
                    vec1d dust_law_qso = Temple_attenuation(rlam);
                    //vec1d dust_law_qso = calzetti2000(rlam);
                    rsed *= e10(-0.4*out.av_qso[i]*dust_law_qso);

                } else { // For galaxies and type 2 agn, initialize flux=0 at all lambda
                    rsed = rsedagn - rsedagn;
                }
            } else { //(cmp = disk or bulge)
                if (no_ir) {
                    ...
                    merge_add(orlam, irlam, orsed, irsed, rlam, rsed); // merge_add defined in egg_utils.hpp
                }
            }
            // Add emission lines --> Lucia, Xavi, Micol: moved before IGM (it was after, see comment when applying attenuation to lines)
            // Don't add emission lines if no_addlines is specified (by LuciaP)
            if (!no_addlines) {
                // Xavi: Add narrow lines only to type 2 AGN
                if (cmp == component::agn && out.agn_type[i] == 2){
                        for (uint_t l : range(out.agn2_lines)){
                            add_emission_line(rlam, rsed, out.agn2_line_lambda[l], llum(i,l), out.agn2_vdisp[i]);
                        }
                } else if (cmp == component::disk) {
                    if (!no_nebular) {
                        for (uint_t l : range(out.lines)) {
            ...
            // Apply IGM absorption to continuum  (and lines)
            if (igm != "none" && igm != "constant") {
                //uint_t lz = lower_bound(rlam, rlam[0]); // Xavi: changed 0.0600 to rlam[0] to avoid Segmentation fault when using the qso template, which starts at approx 0.8 microns
                uint_t lz = lower_bound(rlam, 0.0600); // Original line
            ...
            // Add emission lines  (moved above)
           // if (!no_nebular) {
           //     for (uint_t l : range(out.lines)) {
           //         add_emission_line(rlam, rsed, out.line_lambda[l], llum(i,l), out.vdisp[i]);
           //     }
           // }
            ...
            if (save_sed) {
                ...
                sed_saver.push(
                    ...
                    &save_sed_disk_start,&save_sed_disk_nbyte,&save_sed_agn_start,&save_sed_agn_nbyte,cmp,i,lam,sed]() {
                    ...
                    } else if (cmp == component::agn){
                        save_sed_agn_start[i] = sp;
                        save_sed_agn_nbyte[i] = nval;
                    }
                    ++nwritten;
                });
            }
    ...
    };//end of auto get_flux



    // Micol: replaced out.line_lum_bulge with out.line_lum_bulge_noigm
    // and out.line_lum_disk with out.line_lum_disk_noigm: in the
    // auto get_flux function the lines are added to the continuum before
    // the IGM, so that the IGM is applied correctly to lines at lambda < 1216A
    

    
    if (!no_agn) {
        // Get flux for the AGN component
        print("calling get flux for agn");
        get_flux(out.m_bulge, out.opt_sed_bulge, out.agn2_line_lum_noigm, // out.m and out.opt_sed_bulge are only used in get_opt_sed, so not used for agn
                 out.flux_agn, out.rfmag_agn, true, component::agn);
    }
    
    // Get flux for the bulge
    if (!no_stellar) {
        print("calling get flux for bulge");
        get_flux(out.m_bulge, out.opt_sed_bulge, out.line_lum_bulge_noigm,
            out.flux_bulge, out.rfmag_bulge, true, component::bulge);
    }
    
    // Get flux for the disc
    print("calling get flux for disk");
    get_flux(out.m_disk, out.opt_sed_disk, out.line_lum_disk_noigm,
    ...
    if (save_sed) {
        ...
        fits::write_table(file::remove_extension(seds_file)+"-lookup.fits",
            ...
            "agn_start", save_sed_agn_start, "agn_nbyte", save_sed_agn_nbyte,
            "elem_size", sizeof(float)
        );
    }

    out.flux = out.flux_disk + out.flux_bulge + out.flux_agn;
    out.rfmag = uJy2mag(mag2uJy(out.rfmag_disk) + mag2uJy(out.rfmag_bulge) + mag2uJy(out.rfmag_agn));
};
    ...
    fits::write_table(out_file, ftable(
        out.id, out.ra, out.dec, out.z, out.d, out.m,
        out.sfr, out.rsb, out.m2lcor,
        out.disk_angle, out.disk_radius, out.disk_ratio,
        out.bulge_angle, out.bulge_radius, out.bulge_ratio,
        out.bt, out.m_disk, out.m_bulge, out.av_disk, out.av_bulge, out.av_qso,
		out.passive, out.agn_type, out.Lacc, out.Lx,out.Mbh, out.emline_type, out.NH,
        ...
        out.metallicity, out.metallicity2, out.m_gas, out.m_h2, out.cmd,
		out.metallicity_agn2,out.nh_nlr,out.logU, out.uv_slope_nlr, out.agn2_vdisp
    ));

    // added by Micol to avoid empty columns when using an input catalog
    if (input_cat_file.empty()) {
        fits::update_table(out_file, ftable(out.clustered, out.zb
        ));
    }
    ...
    // Micol: block copied and modified below, 
    // since lines at present are generated only in disks
    //
    //    if (!out.lines.empty()) {
    //        fits::update_table(out_file, ftable(
    ...
    //            // added by Micol
    //            out.line_lum_nodust, out.line_lum_disk_nodust, out.line_lum_bulge_nodust, 
    ...
    if (!out.lines.empty()) {
        fits::update_table(out_file, ftable(
            out.vdisp, out.avlines_disk, out.avlines_bulge,
            out.line_lum, out.line_lum_nodust, 
            out.lines, out.line_lambda
        ));
    }
    
    // Added by Xavi
    if (!out.agn2_lines.empty()) {
        fits::update_table(out_file, ftable(
            out.agn2_line_lum, out.agn2_line_lum_nodust,
            out.agn2_lines, out.agn2_line_lambda
        ));
    }
    ...
    if (!rfbands.empty()) {
        fits::update_table(out_file, ftable(
            out.rfmag, out.rfmag_disk, out.rfmag_bulge, out.rfmag_agn,
...
void print_help() {
    ...
    print("egg-gencat v1.4.0 modified by Micol, Lucia and Xavi");
